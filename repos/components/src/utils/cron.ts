...
export const parseCron = (cron: string) => {
  ...
  const days = parts[5] !== `*` ? parts[5].split(`,`).map((d) => DaysOfWeek[parseInt(d, 10)]) : []
  ...
  if (parts[1].startsWith(`*/`)) {
    ...
    interval = parseInt(parts[1].slice(2), 10)
  }
  else if (parts[2].startsWith(`*/`)) {
    repeat = ERepeatType.hourly
    interval = parseInt(parts[2].slice(2), 10)
  }
  else if (parts[3].startsWith(`*/`)) {
    repeat = ERepeatType.daily
    interval = parseInt(parts[3].slice(2), 10)
  }
  else if (parts[5] !== `*`) {
    ...
    interval = parseInt(parts[4].slice(2), 10)
  }
  ...
  const {
    ...
    switch (repeat) {
      ...
      case ERepeatType.weekly:
        const sdays = days.length > 0 ? days.map((day) => DaysOfWeek.indexOf(day)).join(`,`) : `*`
