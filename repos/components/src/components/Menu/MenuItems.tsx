...
export const MenuItem = (props: TMItem) => {
  const { Menu, item, onCloseMenu, itemsReplace } = props
  ...
  const onItemClick = useInline(async (it: TMenuItem, evt: MouseEvent<HTMLElement>) => {
    ...
    if (it === item) {
      if (
        it.loader &&
        ((!localItems && !loading) || (itemsReplace && !it?.items?.length))
      ) {
        setLoading(true)
    ...
    } else if (!it?.items?.length) open && autoClose && onMenuClose(evt)
    /**
  ...
  const showMenu =
    !disabled && !itemsReplace && Menu && (items?.length || localItems?.length)
  const Icn = StartIcon ? (
    StartIcon
  ) : isParent ? (
    ChevronRightIcon
  ) : (
    <StarCircleIcon sx={{ fontSize: `18px` }} />
  )
  ...
  return (
    <>
      ...
      >
        ...
        {(label && (
          <ListItemText className='tdsk-menu-item-label'>{label}</ListItemText>
        )) ||
          null}
