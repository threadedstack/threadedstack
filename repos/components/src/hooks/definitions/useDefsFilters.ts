...
const getDefinitions = (props: THDefsFilters): TFilteredDefs => {
  const { search, parentDef, definitions } = props
  ...
  const filtered = parentDef?.owner.map((owner) =>
    definitions.filter((def) => def.id !== parentDef.id && def?.parents?.includes(owner))
  )
...
export const useDefsFilters = (props: THDefsFilters) => {
  const { parentDef, definitions } = props
