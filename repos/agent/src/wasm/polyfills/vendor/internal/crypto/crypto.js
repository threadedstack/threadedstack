...
export class Hash {
  ...
  update(data, inputEncoding) {
    ...
    const b = (typeof data === 'string' && inputEncoding === 'hex') 
      ? hexToBytes(data) 
      : toBytes(data);
...
export function random_fill(buffer, offset, size) {
  ...
  if (offset < 0 || size < 0 || offset + size > byteLength) {
    throw new RangeError('The value of "offset" is out of range. It must be >= 0 && <= buffer.byteLength - size.');
  }
...
function getHashInfo(alg) {
  switch (alg) {
    case 'sha1': return { outputLen: 20, blockLen: 64, func: sha1 };
    case 'sha256': return { outputLen: 32, blockLen: 64, func: sha256 };
    case 'sha512': return { outputLen: 64, blockLen: 128, func: sha512 };
    default: return null;
  }
}
...
function sha512(message) {
  ...
  const len = message.length * 8;
  const kLen = (message.length + 17) > 128 ? (Math.ceil((message.length + 17) / 128) * 128) : 128;
  const padded = new Uint8Array(kLen);
  padded.set(message);
  padded[message.length] = 0x80;
...
export class Cipher {
  constructor(algorithm, key, iv) {
    ...
    if (!this.alg.includes('aes-256')) {
      throw new Error(`Only 'aes-256-cbc' and 'aes-256-gcm' are fully supported in this pure-JS polyfill.`);
    }
  ...
  getAuthTag() {
    if (!this._isGCM || !this._gcmState.authTag) throw new Error('Auth tag not available. Call final() first.');
    return this._gcmState.authTag;
  }

  update(data, inputEncoding, outputEncoding) {
    const raw = typeof data === 'string' ? text_encode(data, inputEncoding || 'utf8') : new Uint8Array(data);
    let output;
...
export function scrypt_sync(password, salt, keylen, options) {
  ...
  // 1. Validation
  if (N <= 1 || (N & (N - 1)) !== 0) throw new Error('Scrypt: N must be a power of 2 greater than 1');
