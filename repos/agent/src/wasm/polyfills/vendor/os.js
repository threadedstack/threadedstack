...
import { text_encode } from './internal/encoding';

/**
 * Returns the estimated memory usage in bytes.
 * In StarlingMonkey/SpiderMonkey-Wasm, precise memory tracking usually requires
 * a host call, but we can access the total reserved heap via the
 * performance API if available, or return the standard GC heap size.
 */
function memorySize() {
  if (
...
function freemem() {
  ...
  return totalmem() - memorySize() * 65536;
}
