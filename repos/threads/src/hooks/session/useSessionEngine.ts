...
export function useSessionEngine(sessionId: string | null) {
  ...
  useEffect(() => {
    ...
    const engine = SessionEngine.create(sessionId, {
      onAST: (doc) => setEngineAst(sessionId, doc),
      onFeedEvents: (events) => appendFeedEvents(sessionId, events),
    })
    registerEngine(sessionId, engine)
