...
const overrideEnvs = process.env.TDSK_TH_OVERRIDE_ENVS
...
export const loadConfig = () => {
  ...
  return {
    aliases,
    environment: nodeEnv,
    port: Number.parseInt(process.env.TDSK_TH_PORT || `5887`, 10),
    envs: {
      ...envs,
      [`process.env.NODE_ENV`]: JSON.stringify(process.env.NODE_ENV),
      [`process.env.TDSK_TH_APP_VERSION`]: JSON.stringify(
        process.env.TDSK_TH_APP_VERSION || version
      ),
