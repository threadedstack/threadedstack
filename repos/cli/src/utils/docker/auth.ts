...
const passEnv = (data: Record<string, string>) => {
  const { NPM_TOKEN, GIT_TOKEN, NPM_AUTH_TOKEN, DOCKER_PASSWORD, DOCKER_AUTH_PASSWORD } =
    data

  return (
    DOCKER_PASSWORD || DOCKER_AUTH_PASSWORD || GIT_TOKEN || NPM_TOKEN || NPM_AUTH_TOKEN
  )
}

const userEnv = (data: Record<string, string>) => {
  const { NPM_USER, GIT_USER, NPM_AUTH_USER, DOCKER_USER, DOCKER_AUTH_USER } = data

  return DOCKER_USER || DOCKER_AUTH_USER || GIT_USER || NPM_USER || NPM_AUTH_USER
}
