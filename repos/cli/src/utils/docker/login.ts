...
export const login = (props:TTaskActionArgs) => {
  ...
  !user && taskError(`A docker auth user name or email is required but could not be found`)
  !password && taskError(`A docker auth password or token is required but could not be found`)
