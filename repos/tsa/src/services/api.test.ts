...
vi.mock(`@TSA/constants`, async () => {
  const actual = await vi.importActual(`@TSA/constants`)
  return {
    ...actual,
    MaxRetries: 3,
    RetryDelays: [0, 0, 0],
  ...
  describe(`createThread`, () => {
    it(`should POST to /_/orgs/:orgId/agents/:agentId/threads`, async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        status: 200,
        json: async () => ({ data: { id: `t-new`, name: `TSA session` } }),
      })
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          method: `POST`,
          body: JSON.stringify({ name: `TSA session` }),
        })
      )
      expect(data?.id).toEqual(`t-new`)
      expect(data?.name).toEqual(`TSA session`)
    })
