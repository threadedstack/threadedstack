...
export class ReposApi extends BaseApi {
  ...
  cache: TApiCacheKeys = {
    ...
    listTeam: (teamId: string) => [...this.cache.lists(), teamId] as const,
   ...
   */
  async list(data?: Record<string, any>): Promise<TApiRes<Repo[]>> {
    ...
    return {
      ...resp,
      data: resp.data.map((repo) => new Repo(repo)),
    }
  }
   ...
   */
  async listByTeam(teamId: string): Promise<TApiRes<Repo[]>> {
    return this.list({
      teamId,
