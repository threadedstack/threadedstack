...
describe(`CollectionsApi`, () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks()
    mockFetch = vi.fn()
    collectionsApi.api.mock = mockFetch as any
    onErrorSpy = vi.spyOn(collectionsApi, `_onError`).mockResolvedValue(undefined)
  })
  ...
  describe(`list()`, () => {
    it(`should GET the project-scoped path, use cache.list(orgId, projectId) as the queryKey, and return the raw data array`, async () => {
      ...
      const resp = await collectionsApi.list(`org-1`, `proj-1`)
      ...
      expect(queryOpts.queryKey).toEqual(collectionsApi.cache.list(`org-1`, `proj-1`))
    ...
    it(`should call _onError with 'Failed to load Collections list' on error`, async () => {
      ...
      await collectionsApi.list(`org-1`, `proj-1`)
  ...
  describe(`create()`, () => {
    it(`should POST to the base scoped path with the body and return the response unwrapped`, async () => {
      ...
      const resp = await collectionsApi.create(`org-1`, `proj-1`, payload)
    ...
    it(`should call _onError with 'Failed to create Collection' on error`, async () => {
      ...
      await collectionsApi.create(`org-1`, `proj-1`, { name: `New` })
  ...
  describe(`update()`, () => {
    it(`should PUT to the path with the collection name appended and return the response unwrapped`, async () => {
      ...
      const resp = await collectionsApi.update(`org-1`, `proj-1`, `Old`, payload)
    ...
    it(`should call _onError with 'Failed to update Collection' on error`, async () => {
      ...
      await collectionsApi.update(`org-1`, `proj-1`, `Old`, { name: `x` })
  ...
  describe(`delete()`, () => {
    it(`should DELETE the path with the collection name appended and return the raw {success} response`, async () => {
      ...
      const resp = await collectionsApi.delete(`org-1`, `proj-1`, `Old`)
    ...
    it(`should call _onError with 'Failed to delete Collection' on error`, async () => {
      ...
      await collectionsApi.delete(`org-1`, `proj-1`, `Old`)
  ...
  describe(`cache key shape`, () => {
    it(`cache.all() returns ['collections']`, () => {
      expect(collectionsApi.cache.all()).toEqual([`collections`])
    })

    it(`cache.list(orgId, projectId) extends cache.all() with ['list', orgId, projectId]`, () => {
      expect(collectionsApi.cache.list(`org-1`, `proj-1`)).toEqual([
        `collections`,
        `list`,
        `org-1`,
        `proj-1`,
    ...
    it(`cache.detail(id) extends cache.all() with ['detail', id]`, () => {
      expect(collectionsApi.cache.detail(`c1`)).toEqual([`collections`, `detail`, `c1`])
    })
  })
})
