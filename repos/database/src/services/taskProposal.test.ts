 ...
 * exposes `.limit` (for findOpenByDedupeKey / listBacklog). Also covers the
...
describe(`TaskProposal service`, () => {
  ...
  describe(`createIfAbsent`, () => {
    ...
    it(`simulates two concurrent callers racing the same dedupeKey — exactly one row is created`, async () => {
      ...
      mocks.insertReturningFn.mockResolvedValueOnce([fakeRow({ id: `tp_winner1` })])
      ...
      expect(winner.data?.id).toBe(`tp_winner1`)
