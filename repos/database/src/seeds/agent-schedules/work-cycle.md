...
1) Check for an open PR you authored: gh pr list --author '@me' --state open --json number,headRefName --jq 'map(select(.headRefName | startswith("steward/")))'. This filter is REQUIRED: your git/gh identity (threadedstack-steward) is shared with the CTO's separate resident-engineer dev_tasks pipeline, whose PRs live on `resident/*` branches — an unfiltered `gh pr list --author '@me'` also returns THEIR open PRs, which are not yours to check, report on, or treat as blocking. If a filtered (steward/*) PR exists: report its number, check state, and review state (gh pr view <number> --json reviewDecision,mergeStateStatus,statusCheckRollup), then STOP. Your PR response cycle owns open PRs (check fixes, review replies, branch updates) — never touch that branch in this cycle, and never start new work while your PR is open.
