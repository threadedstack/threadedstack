    ...
    agenda: [
      ...
      // CTO became the SOLE groomer.
      {
        ...
        prompt: [
          ...
          `WHEN THE SENSOR BACKLOG RUNS DRY, GENERATE PROACTIVE WORK — the team has caught up on reactive bug-fixes, but a startup's engineering never stops, so keep building toward the company's goals. If, after pruning and grooming, the open board is still below ~6-9, source new engineering work IN THIS ORDER: (1) ADVANCE THE ENGINEERING ROADMAP — read the injected "Company Strategy" + "Plans". The CTO's product/engineering roadmap (the cto-owned initiative plan) is your PRIMARY source of buildable milestones; the company and gtm plans mostly hold business and marketing milestones the engineering team cannot build, so pick the highest-leverage OPEN milestone the engineers CAN move and decompose it into small, sharply-scoped dev_tasks (e.g. a roadmap milestone for a shipped primitive → the docs/admin-UI/website tasks to surface it). (2) HIGH-VALUE, EVIDENCED PLATFORM IMPROVEMENTS — a real performance or reliability weakness, a fragile pattern worth hardening, genuine tech debt slowing the team, or a specific test-coverage gap you VERIFIED (name the untested file/path) — always grounded in real code you read, never speculative. An under-full board more often means the roadmap needs another milestone than that tests are the answer: prefer the highest-leverage improvement you can evidence, and when the roadmap keeps running dry, flag it to the CTO board seat with sendAgentMessage so it charters more roadmap work. Proactive tasks carry NO sourceTaskProposalId (they are not sensor proposals); everything else (small, DoD, atomic) is identical. HARD GUARD: never manufacture busywork to fill the board. A cosmetic refactor, a rename, or a "just in case" abstraction with no evidenced value is worse than an idle engineer — if there is genuinely no high-value proactive work right now, groom what is real, say the board is lean, and stop. Quality and leverage over volume, always.`,
