  ...
  data: {
    ...
    watches: [
      ...
      // agenda-paced (see the doc above), never watch-fast.
      {
        ...
        debounceMs: 600_000,
        prompt: [
          `The active plans changed — review whether the company plan needs action.`,
          `Check your company plan's open milestones and keyResults against the change shown in the matched records: if it is affected, act (advance a milestone, update strategy, or prepare a board decision); if not, say so in one line and stop. The gtm and initiative plans belong to the CMO and CTO — you may report progress on them, but you do not author them.`,
        ].join(`\n`),
      },
    ],
    inbox: { pollMs: 15_000 },
    ...
    selfDirected: {
      minIdleMs: 600_000,
      prompt: `Advance company strategy: refine positioning and segments from research, maintain the company plan's open milestones, and prepare decisions for the board. NEVER resolve a decision you opened this turn.`,
    },
    ...
    agenda: [
      ...
      // CTO became the SOLE groomer.
      {
        ...
        prompt: [
          ...
          `YOU ARE THE SOLE GROOMER for THREE engineers — the scheduled dev-loop is decommissioned, so the team only ever works what YOU groom. Keep the engineers fed WITH REAL WORK: groom every real, buildable item available this cycle — the sensor-detected proposals AND the CTO roadmap's open milestones. ~6-9 open dev_tasks is a healthy full board, but it is NOT a quota to hit at any cost. If the real, buildable work available this cycle totals fewer than that, groom what is real and RUN LEAN — a lean board of genuine feature and bug work beats a full board of manufactured filler. When the board is lean because the roadmap is thin, message the CTO board seat (sendAgentMessage) that the roadmap needs more open milestones. NEVER manufacture tasks — test-coverage, cosmetic, or otherwise — to reach a number: an engineer briefly between real tasks is fine; an engineer building invented work is not.`,
          ...
          `THEN GROOM THE LIVE. For the remaining buildable proposals, decompose and add them (per above) until the board holds ~6-9 open tasks. A large or risky proposal (e.g. versioned SQL migrations) still gets groomed — as a SMALL first slice with a tight DoD, not skipped for being big. Only genuinely blocked work (waiting on an unanswered external decision) stays untouched, and you message the blocker's owner about it.`,
          `WHEN THE SENSOR BACKLOG RUNS DRY, ADVANCE THE ENGINEERING ROADMAP — read the injected "Company Strategy" + "Plans". The CTO's product/engineering roadmap (the cto-owned initiative plan) is your PRIMARY source of buildable feature work: pick the highest-leverage OPEN milestone the engineers CAN move and decompose it into small, sharply-scoped dev_tasks. Title each task consistently so re-grooming the same milestone next cycle dedupes on title instead of duplicating; and if a milestone ALREADY has dev_tasks in flight on the board (see "Dev tasks in flight"), it is being worked — do not re-decompose it. You do NOT invent your own roadmap or derive your own features from the strategy — your proactive feature source is the milestones the CTO board seat authored; that keeps one owner of direction and avoids two engineers building the same thing under different titles. IF the roadmap has no open milestone you can move AND the sensor backlog is dry, the board RUNS LEAN this cycle: say so in one line, message the CTO board seat (sendAgentMessage) that the roadmap needs more open milestones, and STOP. NEVER manufacture standalone test-coverage tasks ("add unit test coverage for <file>") to fill the board — untested files are infinite, and coverage-for-coverage is exactly the manufactured filler that drowns out real product work. Test coverage is written by the engineer as part of a feature's definition of done, and a genuinely critical untested path arrives as a sensor proposal — never as groomed board-fill. The same bar rejects cosmetic refactors, renames, and "just in case" abstractions. A lean board of real work is the correct outcome when there is no real work to add — never a wall of manufactured tasks. Proactive roadmap tasks carry NO sourceTaskProposalId; everything else (small, DoD, atomic) is identical.`,
          `When the board is healthily full — or every remaining scanned proposal is groomed/abandoned/blocked AND there is no high-value proactive work — say so in one line and stop.`,
        ].join(`\n`),
      },
