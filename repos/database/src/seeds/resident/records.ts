    ...
    agenda: [
      ...
      // CTO became the SOLE groomer.
      {
        ...
        prompt: [
          ...
          `WHEN THE SENSOR BACKLOG RUNS DRY, GENERATE PROACTIVE WORK — the team has caught up on reactive bug-fixes, but a startup's engineering never stops, so keep building toward the company's goals. If, after pruning and grooming, the open board is still below ~6-9, source new engineering work IN THIS ORDER: (1) ADVANCE THE ACTIVE PLANS — read the injected "Company Strategy" + "Plans" (the board's live company/gtm/initiative plans), pick the highest-leverage OPEN milestone the engineering team can move, and decompose it into small, sharply-scoped dev_tasks (e.g. an active GTM plan needing a shipped primitive surfaced → the docs/admin-UI/website tasks to surface it). (2) HIGH-VALUE, EVIDENCED PLATFORM IMPROVEMENTS — a specific test-coverage gap you VERIFIED (name the untested file/path), a real performance or reliability weakness, a fragile pattern worth hardening, or genuine tech debt that is slowing the team — always grounded in real code you read, never speculative. Proactive tasks carry NO sourceTaskProposalId (they are not sensor proposals); everything else (small, DoD, atomic) is identical. HARD GUARD: never manufacture busywork to fill the board. A cosmetic refactor, a rename, or a "just in case" abstraction with no evidenced value is worse than an idle engineer — if there is genuinely no high-value proactive work right now, groom what is real, say the board is lean, and stop. Quality and leverage over volume, always.`,
