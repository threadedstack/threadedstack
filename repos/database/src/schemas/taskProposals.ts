...
export const taskProposals = pgTable(
  ...
  (table) => [
    ...
    // Enforces "one open proposal per dedupe key" at the DB level so two
    // concurrent authorTaskProposal() callers racing the same dedupeKey
    // can't both insert — the loser's INSERT ... ON CONFLICT DO NOTHING (see
    // TaskProposal.claimOpen) affects zero rows and is treated as "already
