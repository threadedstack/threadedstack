...
- [ ] **Self-extension surface (spec §5.1)**: `authorFunction` — agent submits `{name, description, language, content}`; fail-closed deterministic scan (the `context.scan` scanner over the content) ⇒ reject or create the Function project-scoped (isolate-bound by construction); caller-stamped provenance in the function record meta; unit + parity tests incl. malicious-content reject. Skill self-authoring needs NO new surface (the existing proposal→scan→curate→promote pipeline + sandbox skills configMap) — wire the exec agents: skill-authoring mandate in prompts + `agent_skills` attachment on promotion verified for exec sandboxes (cite the configMap mount).
- [ ] Exec prompts (follow-up commit on the board branch or here): the primitives-faculty section gains the FAST PATH ("author a Function yourself via authorFunction; author a Skill when you repeat a methodology") alongside the existing dev-loop deep path.
